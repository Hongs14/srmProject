       ...
       aria-expanded="false" aria-controls="collapsePage">
          ...
          <div class="bg-white py-2 collapse-inner rounded">
            ...
            <c:if test="${sessionScope.loginUser.userOgdp eq '북북' or sessionScope.loginUser.userOgdp eq '한국소프트'}">            
	            <a class="collapse-item" href="${pageContext.request.contextPath}/notice/list?sysNo=JHJ">북북STORE 공지</a>
            </c:if>
            <c:if test="${sessionScope.loginUser.userOgdp eq '사슈즈' or sessionScope.loginUser.userOgdp eq '한국소프트'}">
	            <a class="collapse-item" href="${pageContext.request.contextPath}/notice/list?sysNo=KTH">사슈즈STORE 공지</a>
            </c:if>
            <c:if test="${sessionScope.loginUser.userOgdp eq '에이치알컴퍼니' or sessionScope.loginUser.userOgdp eq '한국소프트'}">
	            <a class="collapse-item" href="${pageContext.request.contextPath}/notice/list?sysNo=KHR">KHR시스템 공지</a>
            </c:if>
            <c:if test="${sessionScope.loginUser.userOgdp eq '지에이치컴퍼니' or sessionScope.loginUser.userOgdp eq '한국소프트'}">
